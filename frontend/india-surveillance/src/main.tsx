...
function App() {
  ...
  async function reviewSubmission(id: string, status: string) {
    if (!session) return; await api(`/daily-submissions/${id}/review`, session.access_token, { method: 'POST', body: JSON.stringify({ review_status: status, reviewed_by: session.user.name, notes: `Marked ${status}` }) }); await load(session.access_token);
  }
  async function reviewCase(id: string) {
    if (!session) return; await api(`/event-cases/${id}/review`, session.access_token, { method: 'POST', body: JSON.stringify(caseReview) }); await load(session.access_token);
  }
  async function triageReport(id: string) {
    if (!session) return; await api(`/reports/${id}/triage`, session.access_token, { method: 'POST', body: JSON.stringify(triage) }); await load(session.access_token);
  }
  async function closeReport(id: string) {
    if (!session) return; await api(`/reports/${id}/close`, session.access_token, { method: 'POST', body: JSON.stringify({ closure_note: `Closed by ${session.user.name}` }) }); await load(session.access_token);
  }
  async function createUser(event: React.FormEvent) {
    event.preventDefault(); if (!session) return;
    await api('/users', session.access_token, { method: 'POST', body: JSON.stringify(userForm) }); setUserForm((f: any) => ({ ...f, name: '', username: '', department_id: '' })); await load(session.access_token);
  }
  async function acknowledge(id: string, status: string) {
    if (!session) return; await api(`/notifications/${id}/acknowledge`, session.access_token, { method: 'POST', body: JSON.stringify({ status }) }); await load(session.access_token);
  }
  async function saveAiConfig(event: React.FormEvent) {
    event.preventDefault(); if (!session) return;
    await api('/ai/providers', session.access_token, { method: 'POST', body: JSON.stringify(aiConfig) }); setAiConfig((f: any) => ({ ...f, api_key: '' })); await load(session.access_token);
  }
  async function runAiAssist(event: React.FormEvent) {
    event.preventDefault(); if (!session) return;
    setAiResult(await api('/ai/assist', session.access_token, { method: 'POST', body: JSON.stringify(aiAssist) })); await load(session.access_token);
  }
