...
type ViewKey = 'dashboard' | 'submissions' | 'cases' | 'ai' | 'governance';
...
function routeFromHash(): ViewKey {
  ...
  return views.some(([key]) => key === candidate) ? candidate : 'dashboard';
}
...
function App() {
  ...
  const [activeView, setActiveView] = useState<ViewKey>(routeFromHash());
  ...
  useEffect(() => {
    const onHash = () => setActiveView(routeFromHash());
  ...
  if (!session) return <div className="login-shell"><div className="login-grid"><section className="login-brand surface-glass lazy-in"><p className="eyebrow">National patient safety intelligence</p><div className="brand-lockup"><span className="brand-mark">PS</span><div><p className="brand-kicker">India Patient Safety Surveillance System</p><h1 className="brand-title">Operational surveillance for departments, facilities, states, and national review.</h1></div></div><p className="lede">Daily surveillance, event-case review, CAPA tracking, trend analytics, and AI-assisted classification in one operational workflow.</p><div className="brand-metrics"><div className="brand-metric"><strong>Facility</strong><span>department reporting and review</span></div><div className="brand-metric"><strong>State</strong><span>signal triage, escalation, and learning</span></div><div className="brand-metric"><strong>National</strong><span>governance, benchmarking, and prevention</span></div></div></section><div className="login-panel surface-glass lazy-in"><p className="eyebrow">Secure access</p><h1 className="login-title">Facility and state login</h1><p className="lede">Departments submit daily data. Managers review cases, CAPA, analytics, and AI-assisted classification.</p><form onSubmit={doLogin} className="login-form"><label>Username<input value={username} onChange={(e) => setUsername(e.target.value)} /></label><label>Password<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} /></label><button type="submit">Login</button></form><div className="demo-credentials"><strong>Seed accounts</strong><small><code>tmk-ed</code>, <code>tmk-icu</code>, <code>ka-fso</code>, <code>state-ka</code>, <code>national</code>, <code>admin</code></small><small>Password: <code>pass123</code></small></div>{loginError && <p className="error-text">{loginError}</p>}<AppFooter compact /></div></div></div>;

  return <div className="page-shell">
    <header className="hero compact-hero lazy-in"><div><p className="eyebrow">Logged in as {role}</p><h1>Safety surveillance command center</h1><p className="lede">{session.user.name}</p></div><div className="hero-card surface-glass"><strong>{session.user.facility_id || session.user.state || 'National scope'}</strong><p>{apiBase}</p><div className="action-row">{installPrompt && <button onClick={() => (installPrompt as any).prompt()}>Install app</button>}<button onClick={() => void load(session.access_token)}>Refresh</button><button onClick={() => setSession(null)}>Logout</button></div></div></header>
    {errorBanner && <section className="error-banner">{errorBanner}</section>}
    {notice && <section className="success-banner">{notice}</section>}
    <nav className="view-nav">
      {views.map(([key, label]) => <button key={key} className={`view-tab ${activeView === key ? 'active' : ''}`} onClick={() => navigateTo(key)}>{label}</button>)}
    </nav>
    ...
    {activeView === 'dashboard' && <section className="page-view dashboard-view">
      <section className="panel-grid panel-grid-wide">
      <Panel title="Operations queue" subtitle="Submissions, reports, and high-risk cases needing attention"><div className="chips summary-chips"><span className="chip">Pending submissions: {data.submissions.filter((x: any) => x.review_status === 'submitted').length}</span><span className="chip">Open reports: {data.reports.filter((x: any) => x.status !== 'closed').length}</span><span className="chip">Open cases: {data.eventCases.filter((x: any) => x.closure_status !== 'closed').length}</span><span className="chip">High-risk cases: {data.eventCases.filter((x: any) => ['severe', 'sentinel'].includes(x.severity_level)).length}</span></div><div className="report-filters"><label>Search reports<input value={reportSearch} onChange={(e) => setReportSearch(e.target.value)} placeholder="ID, domain, summary" /></label><label>Severity<select value={reportSeverity} onChange={(e) => setReportSeverity(e.target.value)}><option value="all">All</option><option value="moderate">Moderate</option><option value="severe">Severe</option><option value="sentinel">Sentinel</option></select></label></div><div className="queue-table-shell"><table className="queue-table"><thead><tr><th>Report</th><th>Domain</th><th>Summary</th><th>Status</th><th>Assigned</th><th>Severity</th></tr></thead><tbody>{filteredReports.map((report: any) => <tr key={report.report_id} className={selectedReportId === report.report_id ? 'selected' : ''} onClick={() => setSelectedReportId(report.report_id)}><td>{report.report_id}</td><td>{humanize(report.domain)} / {humanize(report.deviation_class)}</td><td>{report.summary}</td><td>{report.status}</td><td>{report.assigned_to || 'unassigned'}</td><td><span className={`pill severity-${report.severity}`}>{report.severity}</span></td></tr>)}</tbody></table></div><div className="queue-mobile-list">{filteredReports.map((report: any) => <button key={report.report_id} className={`report-card selectable compact-card ${selectedReportId === report.report_id ? 'selected' : ''}`} onClick={() => setSelectedReportId(report.report_id)}><div className="report-header"><span>{report.report_id}</span><span className={`pill severity-${report.severity}`}>{report.severity}</span></div><strong>{humanize(report.domain)} / {humanize(report.deviation_class)}</strong><p>{report.summary}</p><small>{report.status} | {report.assigned_to || 'unassigned'}</small></button>)}</div>{canManageReports && selectedReportId && <div className="subsection action-block"><h3>Report action</h3><div className="daily-form"><label>Status<select value={triage.status} onChange={(e) => setTriage({ ...triage, status: e.target.value })}><option value="triaged">Triaged</option><option value="investigating">Investigating</option></select></label><label>Assigned to<input value={triage.assigned_to} onChange={(e) => setTriage({ ...triage, assigned_to: e.target.value })} /></label><label>State cell<input value={triage.state_cell} onChange={(e) => setTriage({ ...triage, state_cell: e.target.value })} /></label></div><div className="action-row"><button onClick={() => void triageReport(selectedReportId)}>Save triage</button><button onClick={() => void closeReport(selectedReportId)}>Close report</button></div></div>}</Panel>
    ...
    {activeView === 'submissions' && <section className="page-view submissions-view"><section className="page-head"><div><p className="eyebrow">Operational reporting</p><h2 className="route-title">Submissions</h2></div><p className="route-copy">Department daily surveillance intake and manager review workflow.</p></section><section className="panel-grid">
    ...
    {activeView === 'cases' && <section className="page-view cases-view"><section className="page-head"><div><p className="eyebrow">Learning cases</p><h2 className="route-title">Cases</h2></div><p className="route-copy">Event-level intake, CAPA ownership, and closure quality tracking.</p></section><section className="panel-grid">
    ...
    {activeView === 'ai' && <section className="page-view ai-view"><section className="page-head"><div><p className="eyebrow">AI workspace</p><h2 className="route-title">AI</h2></div><p className="route-copy">Trend interpretation, provider configuration, and assisted case classification.</p></section><section className="panel-grid">
    ...
    {activeView === 'governance' && <section className="page-view governance-view"><section className="page-head"><div><p className="eyebrow">Oversight and control</p><h2 className="route-title">Governance</h2></div><p className="route-copy">Notifications, policy state, audit activity, and access administration.</p></section><section className="panel-grid">
...
function Panel(props: { title: string; subtitle: string; children: React.ReactNode }) { return <div className="panel surface-glass lazy-in"><div className="panel-head"><h2>{props.title}</h2><p>{props.subtitle}</p></div>{props.children}</div>; }
function Metric(props: { label: string; value: number }) { return <div className="metric-card surface-glass lazy-in"><span>{props.label}</span><strong>{props.value}</strong></div>; }
function AppFooter(props: { compact?: boolean }) {
  return <footer className={`app-footer ${props.compact ? 'compact' : ''}`}>
    <p><strong>Concept and originating framework:</strong> Dr Siddalingaiah H S, Professor, Community Medicine, Shridevi Institute of Medical Sciences and Research Hospital, Tumkur.</p>
    <p>ORCID: <a href="https://orcid.org/0000-0002-4771-8285" target="_blank" rel="noreferrer">0000-0002-4771-8285</a></p>
  </footer>;
}
