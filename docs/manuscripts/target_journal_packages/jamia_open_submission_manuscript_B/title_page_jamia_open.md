# Title Page

## Article type

Research and Applications

## Full title

A unified domain taxonomy and process-deviation framework for multidomain patient safety intelligence

## Short title

Multidomain safety taxonomy

## Authors

Dr. Siddalingaiah H S, Project SentinelCare, Bengaluru, India

## Corresponding author

Dr. Siddalingaiah H S  
Project SentinelCare  
Bengaluru, India  
hssling@yahoo.com  
To be supplied in journal submission portal

## Keywords

patient safety; taxonomy; clinical decision support; diagnostic safety; medication safety; health informatics

## Word counts

- Abstract: journal-structured abstract
- Main text: target within JAMIA Open `Research and Applications` limit of 4000 words excluding references and acknowledgments

## Tables and figures

- Tables: 4
- Figures: 3

## Data and code

- Code repository: https://github.com/hssling/sentinelcare-mvp
- Data availability statement included in manuscript

## Funding

No specific external funding was received for this framework development, manuscript preparation, or prototype implementation work.

## Conflict of interest

Dr. Siddalingaiah H S is the lead architect and maintainer of the SentinelCare concept, manuscripts, and prototype repository. No commercial product revenue is reported for the work described in this manuscript.

## Prior presentation

None.
